...
public:
    ...
    {
        if (lev <= th) std::cout << str << std::endl;
    }
...
public:
    ...
    System(const std::string& strVocFile, const std::string& strSettingsFile, const eSensor sensor,
           const std::string& seg_net_file = std::string(), const bool bUseViewer = false, const int initFr = 0,
    ...
    // }
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr getPointCloudMap()
    {
...
private:
    ...
    std::shared_ptr<PointCloudMapping> mPointCloudMapper;
