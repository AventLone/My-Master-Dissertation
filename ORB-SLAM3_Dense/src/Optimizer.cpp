                                       ...
                                       const bool& bFixScale)
{
         ...
         mit++)
    {
        ...
        for (std::set<KeyFrame*>::const_iterator sit = spConnections.begin(), send = spConnections.end(); sit != send;
             sit++)
        {
...
{
    Verbose::PrintMess("Opt_Essential: There are " + std::to_string(vpFixedKFs.size()) + " KFs fixed in the merged map",
                       Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "Opt_Essential: There are " + std::to_string(vpFixedCorrectedKFs.size()) + " KFs fixed in the old map",
        Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "Opt_Essential: There are " + std::to_string(vpNonFixedKFs.size()) + " KFs non-fixed in the merged map",
        Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "Opt_Essential: There are " + std::to_string(vpNonCorrectedMPs.size()) + " MPs non-corrected in the merged map",
        Verbose::VERBOSITY_DEBUG);
                            ...
                            const bool bAllPoints)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            Verbose::PrintMess(
                "    Remove point -> i2: " + std::to_string(i2) + "; bAllPoints: " + std::to_string(bAllPoints),
                Verbose::VERBOSITY_DEBUG);
            continue;
        }
                                ...
                                int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
        {
            ...
            if (!VP1 || !VV1 || !VG1 || !VA1 || !VP2 || !VV2 || !VG2 || !VA2)
            {
                std::cerr << "Error " << VP1 << ", " << VV1 << ", " << VG1 << ", " << VA1 << ", " << VP2 << ", " << VV2
                          << ", " << VG2 << ", " << VA2 << std::endl;
                continue;
            }
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame* pFrame, bool bRecInit)
{
    ...
    pFrame->SetImuPoseVelocity(
        VP->estimate().Rwb.cast<float>(), VP->estimate().twb.cast<float>(), VV->estimate().cast<float>());
    Vector6d b;
    b << VG->estimate(), VA->estimate();
    ...
    pFrame->mpcpi = new ConstraintPoseImu(
        VP->estimate().Rwb, VP->estimate().twb, VV->estimate(), VG->estimate(), VA->estimate(), H);
