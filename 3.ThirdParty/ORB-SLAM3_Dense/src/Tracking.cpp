...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat& imRGB, const cv::Mat& imD, const double& timestamp, string filename)
{
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(
            mImGray, imDepth, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if (mSensor == System::RGBD || mSensor == System::IMU_RGBD)
    {
        pKF->imgRGB = this->imgRGB.clone();
        pKF->imgDepth = this->imgDepth.clone();
        if (pKF->imgRGB.size() != pKF->imgDepth.size())
        {
            // maek error message red
            throw std::runtime_error("\033[1;31mERROR: imgRGB and imgDepth have different sizes!\033[0m");
        }
    }
