...
void LoopClosing::CorrectLoop()
{
...

    {
        ...
        for (std::vector<KeyFrame*>::iterator vit = mvpCurrentConnectedKFs.begin(), vend = mvpCurrentConnectedKFs.end();
             vit != vend;
             vit++)
        {
            KeyFrame* pKFi = *vit;
    ...
    mPointCloudMapper->mIsUpdating = false;   // 强制让已有的更新停止，进行新的
    mPointCloudThread =
        std::thread(&PointCloudMapping::updatePointCloud, mPointCloudMapper, std::ref(*mpCurrentKF->GetMap()));
    mPointCloudThread.detach();
    std::cout << "Point cloud updated!" << std::endl;
    /******************************/
...
void LoopClosing::MergeLocal2()
{
    ...
    std::vector<MapPoint*> vpCheckFuseMapPoint;   // MapPoint vector from current map to allow to fuse duplicated points
...
void LoopClosing::CheckObservations(std::set<KeyFrame*>& spKFsMap1, std::set<KeyFrame*>& spKFsMap2)
{
    ...
    for (KeyFrame* pKFi1 : spKFsMap1)
    {
        ...
        {
            std::cout << "CHECK-OBS: KF " << pKFi1->mnId << " has matched MP with " << mMatchedMP.size()
