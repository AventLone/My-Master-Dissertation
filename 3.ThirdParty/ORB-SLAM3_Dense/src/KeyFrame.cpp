...
KeyFrame::KeyFrame()
    : mnFrameId(0), mTimeStamp(0), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS), mfGridElementWidthInv(0),
      ...
      N(0), mvKeys(static_cast<std::vector<cv::KeyPoint>>(NULL)),
      mvKeysUn(static_cast<std::vector<cv::KeyPoint>>(NULL)), mvuRight(static_cast<std::vector<float>>(NULL)),
      mvDepth(static_cast<std::vector<float>>(NULL)), mnScaleLevels(0), mfScaleFactor(0), mfLogScaleFactor(0),
...
void KeyFrame::PostLoad(std::map<long unsigned int, KeyFrame*>& mpKFid, std::map<long unsigned int, MapPoint*>& mpMPid,
