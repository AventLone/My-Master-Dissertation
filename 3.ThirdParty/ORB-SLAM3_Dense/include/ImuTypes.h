...
public:
    ...
    Bias(float b_acc_x, float b_acc_y, float b_acc_z, float b_ang_vel_x, float b_ang_vel_y, float b_ang_vel_z)
...
public:
    Calib(const Sophus::SE3<float>& Tbc, float ng, float na, float ngw, float naw)
    {
    ...
    void Set(const Sophus::SE3<float>& sophTbc, float ng, float na, float ngw, float naw);
...
public:
    ...
    void IntegrateNewMeasurement(const Eigen::Vector3f& acceleration, const Eigen::Vector3f& angVel, float dt);
    void Reintegrate();
